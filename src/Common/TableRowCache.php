...
abstract class TableRowCache extends Table
{
    ...
    {
        return $this->unserialize($this->_context->getMemcached()->get($this->getKey($id)));
    }
    ...
    public function getCaches(array $ids)
    {
        ...
        $rows = $this->_context->getMemcached()->getMulti(array_keys($keys), Memcached::GET_PRESERVE_ORDER);
    ...
    {
        $key = $this->getKey($id);
        return $this->_context->getMemcached()->set($key, $this->serialize($row), $this->_expiration);
    }
    ...
    public function setCaches(array $rows)
    {
        ...
        return $this->_context->getMemcached()->setMulti($data, $this->_expiration);
    }
    ...
    protected function delCaches(array $ids): bool
    {
        ...
        return $this->_context->getMemcached()->deleteMulti($key);
    }
